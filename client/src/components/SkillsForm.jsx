  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='flex gap-2'>
        <input type="text" placeholder='Enter a skill (e.g, JavaScript, React)' className='flex-1 px-3 py-2 text-sm' onChange={(e)=>setNewSkill(e.target.value)} value={newSkill} onKeyDown={handleKeyPress}/>
      ...
      {data?.length>0?(
        <div className='flex flex-wrap gap-2'>
          {data?.map((skill,index)=>(
            ...
            <button onClick={()=>removeSkill(index)} className='ml-1 hover:bg-blue-200 rounded-full p-0.5 transition-colors'><X className='w-3 h-3'/></button>
            </span>
          ))}
        </div>
      ):(
...
<div className='bg-blue-50 p-3 rounded-lg'>
  <p className='text-sm text-blue-800'>
    <strong>Tip:</strong>Add 8-12 relevant skills. Include programming languages, frameworks, databases, tools, and technologies.
  </p>
</div>
    </div>
  )
